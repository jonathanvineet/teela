...
contract Rental {
    ...
    event Rented(uint256 indexed id, address renter, uint256 days, uint256 amount);
    ...
    function rentItem(uint256 id, uint256 days) external payable {
        Item memory it = items[id];
        require(it.listed, "not listed");
        uint256 total = it.pricePerDay * days;
        require(msg.value == total, "incorrect payment");
        payable(it.owner).transfer(msg.value);
        emit Rented(id, msg.sender, days, total);
    }
}
